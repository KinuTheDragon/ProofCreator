...
function getNodeOutput(node) {
    ...
    switch (node.type) {
        ...
        case "biconditionalIntroduction": {
            ...
            let output2 = getNodeOutputTo(nodes[node.input2], node);
            if (!output1 || !output2) return null;
            if (output1.at(-1) !== ASSUMING) return null;
            if (output2.at(-1) !== ASSUMING) return null;
            let psi = output1[0];
            let phi = output1[1];
        ...
        case "disjunctionElimination": {
            if (node.input1 === null) return null;
            if (node.input2 === null) return null;
            if (node.input3 === null) return null;
            let output1 = getNodeOutputTo(nodes[node.input1], node);
            let output2 = getNodeOutputTo(nodes[node.input2], node);
            let output3 = getNodeOutputTo(nodes[node.input3], node);
            if (!output1 || !output2 || !output3) return null;
            if (output1.at(-1) !== OR) return null;
            if (output2.at(-1) !== ASSUMING) return null;
            if (output3.at(-1) !== ASSUMING) return null;
            let phi = output1[0];
            let psi = output1[1];
            if (!postfixEquals(output2[1], phi)) return null;
